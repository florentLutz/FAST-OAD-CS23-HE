...
class PerformancesAssembly(om.Group):
    ...
    def setup(self):
        ...
        self.connect("dc_bus_1.voltage", "dc_line_5.voltage_a")
        self.connect("dc_line_1.total_current", "dc_bus_1.current_in_1")
        self.connect("dc_line_5.total_current", "dc_bus_1.current_in_2")

        # DC bus 2
        self.connect("dc_bus_2.voltage", "dc_line_2.voltage_a")
        self.connect("dc_bus_2.voltage", "dc_line_5.voltage_b")
        self.connect("dc_line_2.total_current", "dc_bus_2.current_in_1")
        self.connect("dc_line_5.total_current", "dc_bus_2.current_in_2")

        # DC bus 3
        self.connect("dc_bus_3.voltage", "dc_line_3.voltage_a")
        self.connect("dc_bus_3.voltage", "dc_line_6.voltage_b")
        self.connect("dc_line_3.total_current", "dc_bus_3.current_in_1")
        self.connect("dc_line_6.total_current", "dc_bus_3.current_in_2")

        # DC bus 4
        self.connect("dc_bus_4.voltage", "dc_line_4.voltage_a")
        self.connect("dc_bus_4.voltage", "dc_line_6.voltage_a")
        self.connect("dc_line_4.total_current", "dc_bus_4.current_in_1")
        self.connect("dc_line_6.total_current", "dc_bus_4.current_in_2")

        self.connect("dc_bus_5.voltage", "dc_line_1.voltage_b")
        self.connect("dc_bus_5.voltage", "dc_line_2.voltage_b")
        self.connect("dc_bus_5.voltage", "dc_line_3.voltage_b")
        self.connect("dc_bus_5.voltage", "dc_line_4.voltage_b")
...
def test_assembly():
    ...
    print(
        ...
        * problem.get_val("component.dc_line_1.voltage_a", units="V")[0]
    )
    print(
        problem.get_val("component.dc_line_2.total_current", units="A")[0]
        * problem.get_val("component.dc_line_2.voltage_a", units="V")[0]
    )
    print(
        problem.get_val("component.dc_line_3.total_current", units="A")[0]
        * problem.get_val("component.dc_line_3.voltage_a", units="V")[0]
    )
    print(
        problem.get_val("component.dc_line_4.total_current", units="A")[0]
        * problem.get_val("component.dc_line_4.voltage_a", units="V")[0]
    )

    print("\n=========== DC power start of cable ===========")
    print(
        problem.get_val("component.dc_line_1.total_current", units="A")[0]
        * problem.get_val("component.dc_line_1.voltage_b", units="V")[0]
    )
    print(
        problem.get_val("component.dc_line_2.total_current", units="A")[0]
        * problem.get_val("component.dc_line_2.voltage_b", units="V")[0]
    )
    print(
        problem.get_val("component.dc_line_3.total_current", units="A")[0]
        * problem.get_val("component.dc_line_3.voltage_b", units="V")[0]
    )
    print(
        problem.get_val("component.dc_line_4.total_current", units="A")[0]
        * problem.get_val("component.dc_line_4.voltage_b", units="V")[0]
    )
