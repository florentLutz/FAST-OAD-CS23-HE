...
class SizingInverterHeatSinkDimension(om.ExplicitComponent):
    """
    Computation of the dimension of the heat sink of the inverter (plaque froide in french).
    ...
    def setup(self):
        ...
        self.declare_partials(
            of="data:propulsion:he_power_train:inverter:" + inverter_id + ":heat_sink:length",
            wrt="data:propulsion:he_power_train:inverter:" + inverter_id + ":module:width",
            method="exact",
        )
        self.declare_partials(
            of="data:propulsion:he_power_train:inverter:" + inverter_id + ":heat_sink:width",
            wrt="data:propulsion:he_power_train:inverter:" + inverter_id + ":module:length",
            method="exact",
        )
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        outputs["data:propulsion:he_power_train:inverter:" + inverter_id + ":heat_sink:length"] = (
            3.3 * inputs["data:propulsion:he_power_train:inverter:" + inverter_id + ":module:width"]
        )
        outputs["data:propulsion:he_power_train:inverter:" + inverter_id + ":heat_sink:width"] = (
            1.1
            * inputs["data:propulsion:he_power_train:inverter:" + inverter_id + ":module:length"]
        )
