...
@oad.RegisterSubmodel(
    ...
    "fastga_he.submodel.propulsion.constraints.dc_bus.current.ensure",
)
class ConstraintsCurrentEnsure(om.ExplicitComponent):
    """
    Class that computes the difference between the maximum current seen by the DC bus during the
...
@oad.RegisterSubmodel(
    ...
    "fastga_he.submodel.propulsion.constraints.dc_bus.voltage.ensure",
)
class ConstraintsVoltageEnsure(om.ExplicitComponent):
    """
    Class that computes the difference between the maximum voltage seen by the DC bus during the
    mission and the value used for sizing, ensuring each component works below its maxima.
    """
