...
class PerformancesLosses(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        partials["losses_rectifier", "switching_losses_IGBT"] = 6.0 * np.eye(number_of_points)
        partials["losses_rectifier", "switching_losses_diode"] = 6.0 * np.eye(number_of_points)
        partials["losses_rectifier", "conduction_losses_IGBT"] = 6.0 * np.eye(number_of_points)
        partials["losses_rectifier", "conduction_losses_diode"] = 6.0 * np.eye(number_of_points)
