...
class PerformancesRectifierRelations(om.ImplicitComponent):
    """
    The rectifier is divided between a load side where the power source is and a generator side
    where the rest of the circuit is. This component represents the link between the two sides.
    ...
    def setup(self):
        ...
        self.add_input(
            ...
            val=np.full(number_of_points, np.nan),
            units="V",
            desc="Target voltage at the output side of the rectifier",
        )
        self.add_input(
            "dc_current_out",
            val=np.full(number_of_points, np.nan),
        ...
        self.add_input(
            "efficiency",
            val=np.full(number_of_points, np.nan),
            desc="Efficiency of the rectifier",
        )
