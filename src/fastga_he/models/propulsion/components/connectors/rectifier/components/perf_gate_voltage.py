...
class PerformancesGateVoltage(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input(
            "data:propulsion:he_power_train:rectifier:" + rectifier_id + ":igbt:gate_voltage",
            units="V",
            val=np.nan,
        )
        self.add_input(
            "data:propulsion:he_power_train:rectifier:" + rectifier_id + ":diode:gate_voltage",
            units="V",
            val=np.nan,
        )
        self.add_input(
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        outputs["gate_voltage_igbt"] = np.maximum(
            ...
            * (1.0 + alpha_v_igbt * (igbt_temperature - rectifier_reference_temperature)),
            reference_gate_voltage_igbt / 3,
        )

        outputs["gate_voltage_diode"] = np.maximum(
            reference_gate_voltage_diode
            * (1.0 + alpha_v_diode * (diode_temperature - rectifier_reference_temperature)),
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        partials[
            ...
            + rectifier_id
            + ":properties:voltage_temperature_scale_factor:igbt",
        ] = (igbt_temperature - rectifier_reference_temperature) * reference_gate_voltage_igbt
        partials[
            "gate_voltage_igbt",
            "settings:propulsion:he_power_train:rectifier:"
            + rectifier_id
        ...
        partials[
            ...
            + rectifier_id
            + ":properties:voltage_temperature_scale_factor:diode",
        ] = (diode_temperature - rectifier_reference_temperature) * reference_gate_voltage_diode
        partials[
            "gate_voltage_diode",
            "settings:propulsion:he_power_train:rectifier:"
            + rectifier_id
