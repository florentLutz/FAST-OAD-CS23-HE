...
@oad.RegisterSubmodel(
    SUBMODEL_CONSTRAINTS_CURRENT_DC_SPLITTER,
    "fastga_he.submodel.propulsion.constraints.dc_splitter.current.ensure",
)
class ConstraintsCurrentEnsure(om.ExplicitComponent):
    """
    Class that computes the difference between the maximum current seen by the DC splitter during the
    ...
    def setup(self):
        ...
        self.add_output(
            name="constraints:propulsion:he_power_train:DC_splitter:"
            + dc_splitter_id
            + ":current_caliber",
            units="A",
            val=500.0,
            desc="Ensure the splitter current rating is enough for the maximum it sees during the "
            "mission, constraint is respected if < 0",
        )

        self.declare_partials(
            of="constraints:propulsion:he_power_train:DC_splitter:"
            + dc_splitter_id
            + ":current_caliber",
            wrt=[
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        outputs[
            "constraints:propulsion:he_power_train:DC_splitter:"
            + dc_splitter_id
            + ":current_caliber"
        ] = (
...
@oad.RegisterSubmodel(
    SUBMODEL_CONSTRAINTS_VOLTAGE_DC_SPLITTER,
    "fastga_he.submodel.propulsion.constraints.dc_splitter.voltage.ensure",
)
class ConstraintsVoltageEnsure(om.ExplicitComponent):
    """
    Class that computes the difference between the maximum voltage seen by the DC splitter during the
    mission and the value used for sizing, ensuring each component works below its maxima.
    """
    ...
    def setup(self):
        ...
        self.add_output(
            ...
            units="V",
            val=800.0,
            desc="Ensure the splitter voltage rating is enough for the maximum it sees during the "
            "mission, constraint is respected if < 0",
        )
