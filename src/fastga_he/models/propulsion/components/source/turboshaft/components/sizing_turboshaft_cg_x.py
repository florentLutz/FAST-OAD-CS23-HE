...
class SizingTurboshaftCGX(om.ExplicitComponent):
    def initialize(self):
    ...
    def setup(self):
        ...
        self.add_input(
            name="data:propulsion:he_power_train:turboshaft:" + turboshaft_id + ":nacelle:length",
            val=np.nan,
            units="m",
        )
        self.add_input(
            name="settings:propulsion:he_power_train:turboshaft:"
            + turboshaft_id
            + ":cg_in_nacelle",
            val=0.5,
        ...
        if position == "on_the_wing":
            ...
            self.add_input(
                ...
                val=np.nan,
                units="m",
                desc="Distance between the turboshaft front face and the leading edge",
            )
        ...
        self.add_output(
            ...
            units="m",
            val=2.5,
            desc="X position of the turboshaft center of gravity",
        )
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        cg_in_nacelle = inputs[
            "settings:propulsion:he_power_train:turboshaft:" + turboshaft_id + ":cg_in_nacelle"
        ]
