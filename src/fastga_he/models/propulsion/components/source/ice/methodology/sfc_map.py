...
if __name__ == "__main__":
    ...
    sfc_contour = go.Contour(
        ...
        zmax=np.max(sfc_data),
        zmin=np.min(sfc_data),
    )
    fig.add_trace(sfc_contour)
    fig.update_layout(
        title_text="Reference data for ICE SFC",
        title_x=0.5,
    ...
    rpm_for_regression = rpm_mesh_grid.flatten()
    pme_for_regression = pme_mesh_grid.flatten()
    ...
    fig2.update_layout(
        ...
        title_x=0.5,
        yaxis_title=r"$MEP \space [bar]$",
        xaxis_title=r"$RPM \space [min^{-1}]$",
    )
    fig2.show()
