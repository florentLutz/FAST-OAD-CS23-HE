...
class PerformancesShaftPower(om.ExplicitComponent):
    """Computation of the power required on the shaft of the ducted fan."""
    ...
    def setup(self):
        ...
        self.add_input(
            "convergence:propulsion:he_power_train:ducted_fan:" + ducted_fan_id + ":min_power",
            units="W",
            val=1.0e3,
            desc="Convergence parameter used to aid convergence since, if power is too low in "
            "the network, the code will have trouble converging",
        )

        self.add_output("shaft_power_in", val=10e3, shape=number_of_points, units="W")
