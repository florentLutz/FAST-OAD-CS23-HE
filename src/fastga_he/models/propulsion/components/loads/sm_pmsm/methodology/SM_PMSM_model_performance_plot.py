...
if __name__ == "__main__":
    ...
    S_base = 650135.779  # Base power [W]
    ...
    efficiency_grid = np.zeros((len(Tmax_range), len(RPM_range)))
    Sp_grid = np.zeros((len(Tmax_range), len(RPM_range)))

    kk_t = 0.4278  # Torque constant [Nm/A]
    N_c = 2 * p * q * m
    l_c = Lm * k_lc * k_tb
    S_slot = hs * ls
    ...
    # Loop over torque and RPM values
    for i, Torque in enumerate(Tmax_range):
        for j, RPM in enumerate(RPM_range):
            ...
            # Iron losses
            f = RPM / 30  # Frequency [Hz]

            sqrt_f = np.sqrt(f)
            sqrt_Bm = np.sqrt(B_m)

            P_iron = 0
            for k in range(4):  # k = power of sqrt(f)
                for b in range(4):  # b = power of sqrt(Bm)
                    coeff = IRON_LOSSES_COEFF[k][b]
                    term = coeff * (sqrt_f ** (k + 1)) * (sqrt_Bm ** (b + 1))
                    P_iron += term

            # Apply scaling factor if needed (e.g., 224.88)
            P_iron *= W_mot

            P_loss = P_j + P_iron
            Pele = Pem + 2 * P_loss
            Pmec = Pem - 2 * P_mec_loss  # Mechanical output power
    ...
    plt.plot([min(RPM_range), 15000], [166, 166], color="red", linestyle="--", linewidth=1)
    plt.plot([15000, 15000], [min(Tmax_range), 166], color="red", linestyle="--", linewidth=1)
    ...
    plt.title("Specific Power vs RPM and Torque")
    plt.tight_layout()
    plt.show()
