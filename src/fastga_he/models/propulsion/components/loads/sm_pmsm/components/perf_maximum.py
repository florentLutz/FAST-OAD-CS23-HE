...
class PerformancesMaximum(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output(
            "data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":current_ac_max",
            units="A",
            val=500.0,
        ...
        self.add_output(
            "data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":losses_max",
            units="W",
            val=4200.0,
        )
        self.add_output(
    ...
    def setup_partials(self):
        ...
        self.declare_partials(
            of="data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":current_ac_max",
        ...
        self.declare_partials(
            of="data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":voltage_ac_max",
        ...
        self.declare_partials(
            of="data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":torque_max",
            wrt="torque_out",
            method="exact",
            rows=np.zeros(number_of_points),
            cols=np.arange(number_of_points),
        ...
        self.declare_partials(
            of="data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":rpm_max",
            wrt="rpm",
            method="exact",
            rows=np.zeros(number_of_points),
            cols=np.arange(number_of_points),
        ...
        self.declare_partials(
            of="data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":losses_max",
            wrt="power_losses",
            method="exact",
            rows=np.zeros(number_of_points),
            cols=np.arange(number_of_points),
        ...
        self.declare_partials(
            of="data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":shaft_power_max",
            wrt="shaft_power_out",
            method="exact",
            rows=np.zeros(number_of_points),
            cols=np.arange(number_of_points),
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        outputs["data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":current_ac_max"] = np.max(
            inputs["ac_current_rms_in_one_phase"]
        )
        outputs["data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":voltage_ac_max"] = np.max(
            inputs["ac_voltage_peak_in"]
        )
        outputs["data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":torque_max"] = np.max(
            inputs["torque_out"]
        )
        outputs["data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":rpm_max"] = np.max(
            inputs["rpm"]
        )
        outputs["data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":losses_max"] = np.max(
            inputs["power_losses"]
        )
        outputs["data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":shaft_power_max"] = np.max(
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        partials[
            "data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":torque_max", "torque_out"
        ] = np.where(inputs["torque_out"] == np.max(inputs["torque_out"]), 1.0, 0.0)

        partials["data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":rpm_max", "rpm"] = (
            np.where(inputs["rpm"] == np.max(inputs["rpm"]), 1.0, 0.0)
        )

        partials[
            "data:propulsion:he_power_train:SM_PMSM:" + pmsm_id + ":losses_max", "power_losses"
