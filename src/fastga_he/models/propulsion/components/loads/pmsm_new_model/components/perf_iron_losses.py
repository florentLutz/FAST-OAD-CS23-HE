...
DATA_FOLDER_PATH = pth.join(
    pth.join(pth.dirname(pth.dirname(__file__)), "reference"), "iron_losses"
)
...
class PerformancesIronLosses(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.declare_partials(
            of="data:propulsion:he_power_train:ACPMSM:" + pmsm_id + ":iron_power_losses",
            wrt=["rpm"],
            method="fd",
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        W_mot = inputs["data:propulsion:he_power_train:ACPMSM:" + pmsm_id + ":mass"]
        Bm = inputs["data:propulsion:he_power_train:ACPMSM:" + pmsm_id + ":airgap_flux_density"]
        f = RPM * p / 60
        sqrt_f = np.sqrt(f)
        sqrt_Bm = np.sqrt(Bm)
