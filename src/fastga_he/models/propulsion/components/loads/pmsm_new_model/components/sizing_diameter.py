...
class SizingStatorDiameter(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input(
            name="data:propulsion:he_power_train:ACPMSM:" + pmsm_id + ":Tangential_stress",
            val=np.nan,
            units="N/m**2",
        )

        self.add_input(
            name="data:propulsion:he_power_train:ACPMSM:" + pmsm_id + ":torque_rating",
            val=np.nan,
            units="N*m",
        )
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        pmsm_id = self.options["pmsm_id"]
        lambda_ = inputs["data:propulsion:he_power_train:ACPMSM:" + pmsm_id + ":Form_coefficient"]
        sigma = inputs["data:propulsion:he_power_train:ACPMSM:" + pmsm_id + ":Tangential_stress"]
        T_max = inputs["data:propulsion:he_power_train:ACPMSM:" + pmsm_id + ":torque_rating"]
