...
class XfoilPolarMod(ExternalCodeComp):
    ...
    def initialize(self):
        ...
        self.options.declare("inviscid", default=False, types=bool)
    ...
    def _write_script_file(
        ...
        with path(local_resources, _INPUT_FILE_NAME) as input_template_path:
            ...
            parser.mark_anchor("viscosity")
            if self.options["inviscid"]:
                parser.transfer_var("?", 0, 1)
            else:
                parser.transfer_var("VISC", 0, 1)
    ...
    def _read_polar(xfoil_result_file_path: str) -> np.ndarray:
        ...
        if os.path.isfile(xfoil_result_file_path):
            dtypes = [(name, "f8") for name in XfoilPolarMod._xfoil_output_names]
