...
def create_segmented_edges(
    edge_start_x, edge_start_y, edge_end_x, edge_end_y, edge_colors, segments_per_edge=10
):
    """
...
def create_network_plot(
    power_train_file_path: str, layout_prog: str = "dot", orientation: str = "TB"
):
    """
...
def power_train_network_viewer_hv_server(
    ...
    def make_document(doc):
        plot, edge_source, node_source, node_image_sequences, propeller_rotation_sequences = (
            create_network_plot(power_train_file_path, layout_prog, orientation=orientation)
        )
        animation_counter = [0]
...
def start_bokeh_server(
    ...
    Args:
        make_document: Function that creates the Bokeh document
        port (int): Port to run the server on
        address (str): Server address (default: localhost)
        refresh_rate (int): Monitor refresh rate in Hz
        callback_interval (int): Milliseconds between callbacks
        animation_frames (int): Number of animation frames
    """
    server = Server(
        {"/": make_document},
        port=port,
...
if __name__ == "__main__":
    # Example usage
    power_train_file_path = os.path.join(
        DATA_FOLDER_PATH, "simple_assembly_tri_prop_two_chainz.yml"
    )
    power_train_network_viewer_hv_server(
        power_train_file_path=power_train_file_path, layout_prog="dot", orientation="LR"
    )
