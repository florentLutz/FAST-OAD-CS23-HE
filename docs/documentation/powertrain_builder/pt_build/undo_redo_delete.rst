...
``Delete`` button located in the button of the left panel.

Undo and redo
=============
To undo the last action, click on the ``Undo`` button, which revert the last change made to the design. Similarly, to
redo the last undone action, click on the ``Redo`` button, which restore the last change that was undone. The undo or
...
delete mode, click on the ``Delete`` button located in the button of the left panel. Once the delete mode is activated,
