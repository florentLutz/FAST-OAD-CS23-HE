...
RESULTS_SENSITIVITY_PLUs_PLUS_FOLDER_PATH = (
    pathlib.Path(__file__).parent / "results_sensitivity_plus_plus"
)
...
def test_lca_pipistrel_plus_plus_plus_high_bed_cell():
    """
    Tests that contains:
    - A full sizing of a Pipistrel Velis Electro with an enforce of the cell number (not exactly the
        reference case), but with a starting SOC of 100%, with the reference cell.
    ...
    # Write the outputs
    problem.output_file_path = RESULTS_FOLDER_PATH / "pipistrel_plus_plus_plus_out_with_lca.xml"
    problem.write_outputs()


def test_lca_pipistrel_plus_plus_high_bed_cell_soc_start():
    """
    Tests that contains:
    - A full sizing of a Pipistrel Velis Electro with an enforce of the cell number (not exactly the
        reference case), but with a starting SOC of 100%, with the reference cell.
    ...
    for soc_start in soc_start_array:
        ...
        # Write the outputs
        file_name = str(int(soc_start)) + "_soc_start_out.xml"
        problem.output_file_path = RESULTS_SENSITIVITY_PLUs_PLUS_FOLDER_PATH / file_name
        problem.write_outputs()


def test_post_process_results_pipistrel_plus_plus():
    fig = go.Figure()
    ...
    for file in RESULTS_SENSITIVITY_PLUs_PLUS_FOLDER_PATH.iterdir():
        if file.is_file():
            datafile = oad.DataFile(RESULTS_SENSITIVITY_PLUs_PLUS_FOLDER_PATH / file)
            socs_start_mission.append(
                datafile[
                    "data:propulsion:he_power_train:battery_pack:battery_pack_1:SOC_mission_start"
                ].value[0]
...
def test_search_engine_plus_and_plus_plus():
    pipistrel_plus_file_path = RESULTS_FOLDER_PATH / "pipistrel_plus_out_with_lca.xml"
    pipistrel_plus_plus_file_path = RESULTS_FOLDER_PATH / "pipistrel_plus_plus_out_with_lca.xml"

    impact_list_ref_design = ["*"]
    phase_list_ref_design = ["*"]
    component_list_ref_design = ["battery_pack_2"]

    impacts_value_plus = lca_impacts_search_table(
        pipistrel_plus_file_path,
        impact_list_ref_design,
        phase_list_ref_design,
        component_list_ref_design,
        rel=False,
    )
    impacts_value_plus_plus = lca_impacts_search_table(
        pipistrel_plus_plus_file_path,
        impact_list_ref_design,
        phase_list_ref_design,
        component_list_ref_design,
        rel=False,
    )
    print(impacts_value_plus[0] - impacts_value_plus_plus[0])
